...
export function qualityRegenerationAction(checkId: string) {
  ...
  if (checkId.startsWith("image-alt-")) {
    return "画像altを「画像」「hero image」などの汎用説明にせず、記事テーマ、図解内容、読者が見るべき判断軸が分かる短い説明にする。";
  }
  ...
  if (checkId === "generic-phrases") {
    return "近年、重要です、注目されています、わかりやすく解説、today's fast-paced digital landscape、today's rapidly evolving landscape、comprehensive guide、delve into、navigate the complexities、unlock the potential等の汎用句を削り、参照元の事実、一次情報、固有名詞、現場例、判断基準に置き換える。";
  }
